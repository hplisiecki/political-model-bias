...
save_dir = r'/content/drive/MyDrive/Personal/publications/annotations/models/political_emotion_model_pl_roberta'
dropout = 0.6
hidden_dim = 768
...
model.load_state_dict(torch.load('models/political_emotion_model_pl_roberta'))
model.to(device)
model.eval()
...
# save
df.to_csv('political_emotion_results.csv', index=False)

# to excel
df.to_excel('political_emotion_results.xlsx', index=False)
...
emotion_columns = ['Happiness_M', 'Sadness_M', 'Anger_M', 'Disgust_M', 'Fear_M', 'Pride_M', 'Valence_M', 'Arousal_M']
for i, emotion in enumerate(emotion_columns):
    politicians[emotion] = [result[emotion] for result in results]
...
# save
df.to_csv('neutral_sentences_results.csv', index=False)
...
# save
df.to_csv('political_sentences_results.csv', index=False)
